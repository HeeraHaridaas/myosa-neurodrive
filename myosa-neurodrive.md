...
1. Flash the firmware to your MYOSA (ESP32) board using the Arduino IDE.
2. Update the WiFi credentials and your own Blynk template ID / auth token at the top of the sketch.
3. Power on the board. The OLED will show a "Connecting to WiFi..." screen, then begin the 20-second calibration countdown.
4. Hold the steering wheel steady during calibration — this sets your personal baseline steering variance.
5. Once calibration completes, live monitoring begins automatically and the Blynk dashboard starts receiving telemetry.
